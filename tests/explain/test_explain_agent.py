...
# ── Gruppe-B-Gold-Referenz (Few-Shot-Anker, Vergleichsziel der Güte-Bewertung) ─


def test_b_references_load_as_flat_str_map() -> None:
    refs = load_b_references("de")
    assert isinstance(refs, dict)
    assert all(isinstance(k, str) and isinstance(v, str) for k, v in refs.items())


def test_b_reference_present_for_start_r1() -> None:
    refs = load_b_references("de")
    ref = refs.get(dc.START_R1_SURPLUS_OK, "")
    assert ref, "Gold-Referenz für START_R1_SURPLUS_OK fehlt"
    assert any(c.isdigit() for c in ref), "Gold-Referenz nennt keine konkrete Zahl"


def test_agent_loads_b_references(agent: ExplainAgent) -> None:
    assert agent._b_references.get(dc.START_R1_SURPLUS_OK, "")
