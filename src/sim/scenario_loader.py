    ...
    with open(path, encoding="utf-8") as f:
        for line in f:
            ...
            rows.append({
                ...
                "energy_price_ct_kwh": float(parts[7]) if len(parts) > 7 and parts[7] else None,
                "pv_forecast_kw": float(parts[8]) if len(parts) > 8 and parts[8] else None,
            })
    return rows
