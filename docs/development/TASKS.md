...
|---|---|---|---|---|
| K1 | **ProductionRunner** — Orchestriert Adapter, BlockScheduler, RuleEngine, ExplainAgent, EventStore, API in einem Entrypoint | ✅ | `src/production_runner.py` (verdrahtet via `src/main.py`) | asyncio Block-Loop + uvicorn FastAPI |
...
| 2026-05-24 | K2 HA-Dashboards Studie B + E in `bitgrid_dashboard_v1.yaml` ergänzt; `packages/bitgridai.yaml` neu; Runner publiziert Explain-MQTT | Claude Code |
